...
const FGTransferMapping = ({ auth }) => {
    const [tableData, setTableData] = useState([]);
    const [Transfer, setTransfer] = useState({
        FromCode: '', ToCode: '', TransferedBy: auth.empid,branchid:auth.branchid,FGTM_ID:'',Status:''
    })
    ...
    // View Start
    const handleEdit = async (FGTM_ID) => {
        try {
            const alldata = { FGTM_ID, mode: 'SI' }
            const response = await axios.post(`${API_URL}/FGTransferMappingIndividual`, alldata);
            console.log(FGTM_ID)
            ...
            <div>
                 <CTooltip content="Edit">
                    <CIcon
                        ...
                        icon={cilPencil}
                        className='m-2'
                        onClick={() => { handleEdit(params.data.FGTM_ID); setIsModalVisible1(true); }}
                        style={{ color: 'white', background: 'rgb(34,139,34)', borderRadius: '5px' }}
                    />
                </CTooltip>
                {/* <button className='btn' onClick={() => { handleEdit(params.data.FGTM_ID); setIsModalVisible1(true); }}>
    ...
    //     </div>
    // }
    const handleInActive = async (FGTM_ID) => {
        try {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        ...
                        const alldata = { FGTM_ID, mode: 'In-active' }
                        const response = await axios.post(`${API_URL}/InActive_FGtransferMapping`, alldata);
                        console.log(FGTM_ID)
    ...
    const handleUpdate = async (e) => {
        try {
            ...
            console.log(alldata)
            const response = await axios.post(`${API_URL}/Update_FGtransferMapping`, alldata)
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const response = await axios.get(`${API_URL}/FGTransferMappingSelect`);
    ...
    const columdef = [
        ...
        { headerName: "Edit", field: 'Edit', cellRenderer: EditRenderer, width: 80 },
        {
            ...
            cellRenderer: (params) => (
                ...
                //     onClick={() => handleInActive(params.data.FGTM_ID)}
    ...
    const HandleSaveChanges = async () => {
        try {
            ...
            const response = await axios.post(`${API_URL}/FGTransferMapping`, Transfer)
            if (response.status === 200) {
            ...
            >                <div className="modal-dialog modal-lg">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h1 className="modal-title fs-5" id="exampleModalLabel">FG Transfer Mapping Master</h1>
...
FGTransferMapping.propTypes = {
    auth: PropTypes.any.isRequired,
};

export default FGTransferMapping
