...
const Machine = ({ auth }) => {
    ...
    //     </div>
    // }
    const handleEdit = async (Mac_ID) => {
        try {
            const alldata = { Mac_ID, mode: 'SI' }
            const response = await axios.post(`${API_URL}/MachineDetailsIndividual`, alldata);
    ...
    const EditRenderer = (params) => {
        return (
            <div>
                {/* <button className='btn' onClick={() => { handleEdit(params.data.Mac_ID); setIsModalVisible1(true); }}>
                  ...
                  <CTooltip content="Edit">
                    <CIcon
                        ...
                        icon={cilPencil}
                        className='m-2'
                        onClick={() => { handleEdit(params.data.Mac_ID); setIsModalVisible1(true); }}
    ...
    //     </div>
    // }
    const handleInActive = async (Mac_ID) => {
        try {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const alldata = { Mac_ID, mode: 'In-active' }
                        const response = await axios.post(`${API_URL}/InActive_Machine`, alldata);
                        console.log(Mac_ID)
    ...
    const handleUpdate = async (e) => {
        try {
            const alldata = { ...MachineInfo, UpdatedBy: auth.empid }
            console.log(alldata)
            const response = await axios.post(`${API_URL}/UpdateMachine`, alldata)
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const response = await axios.get(`${API_URL}/FetchMachine`);
    ...
    const columdef = [
        ...
        { headerName: " Machine No", field: "MachineNo", filter: true, floatingFilter: true, width: 200 },
        { headerName: "Machine Details", field: "MachineDetails", filter: true, floatingFilter: true, width: 200 },
        { headerName: "Tonnage", field: "Tonnage", filter: true, floatingFilter: true, width: 200 },
        { headerName: "Shot Size", field: "ShotSize", filter: true, floatingFilter: true, width: 200 },
        ...
        { headerName: "Edit", field: 'Edit', cellRenderer: EditRenderer, width: 80 },
        {
            ...
            cellRenderer: (params) => (
                ...
                //     onClick={() => handleInActive(params.data.Mac_ID)}
    ...
    const HandleSaveChanges = async () => {
        ...
        if (MachineInfo.MachineDetails === '') {
            Swal.fire({
                title: 'Warning',
                text: 'Please Enter Machine Details',
                icon: 'warning'
            })
            return
        }
        if (MachineInfo.Tonnage === '') {
            Swal.fire({
                title: 'Warning',
                text: 'Please Enter Tonnage',
                icon: 'warning'
            })
            return
        }
        if (MachineInfo.ShotSize === '') {
            Swal.fire({
                title: 'Warning',
                text: 'Please Enter ShotSize',
                icon: 'warning'
            })
            return
        }
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/AddMachine`, MachineInfo)
            if (response.status === 200) {
                Swal.fire({
                    title: 'Success',
    ...
    const handleUploadData = async () => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                const response = await axios.post(`${API_URL}/Machine_Upload`, formData, {
            ...
            >                <div className="modal-dialog modal-lg">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h1 className="modal-title fs-5" id="exampleModalLabel">Machine Master</h1>
                            ...
                            <div>
                                <div className='d-flex flex-wrap row'>
                                    <div className='col-lg-4 mb-2 '>
                                        ...
                                        <input className='form-control  custom-border' type='text' onChange={(e) => setMachineInfo({ ...MachineInfo, MachineNo: e.target.value })} value={MachineInfo.MachineNo} />
                                    </div>
                                    <div className='col-lg-4 mb-2'>
                                        <label>Machine Details</label>
                                        <input className='form-control  custom-border' type='text' onChange={(e) => setMachineInfo({ ...MachineInfo, MachineDetails: e.target.value })} value={MachineInfo.MachineDetails} />
                                    </div>
                                    <div className='col-lg-4 mb-2'>
                                        <label>Tonnage</label>
                                        <input className='form-control  custom-border' type='text' onChange={(e) => setMachineInfo({ ...MachineInfo, Tonnage: e.target.value })} value={MachineInfo.Tonnage} />
                                    </div>
                                    <div className='col-lg-4 mb-2'>
                                        <label>Shot Size</label>
                                        <input className='form-control  custom-border' type='text' onChange={(e) => setMachineInfo({ ...MachineInfo, ShotSize: e.target.value })} value={MachineInfo.ShotSize} />
                                    </div>
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <div className='d-flex flex-wrap row'>
                                <div className='col-lg-4 mb-2 '>
                                    <label>Machine No</label>
                                    <input className='form-control  custom-border' type='text' onChange={(e) => setMachineInfo({ ...MachineInfo, MachineNo: e.target.value })} value={MachineInfo.MachineNo} disabled />
                                </div>
                                <div className='col-lg-4 mb-2'>
                                    <label>Machine Details </label>
                                    <input className='form-control  custom-border' type='text' onChange={(e) => setMachineInfo({ ...MachineInfo, MachineDetails: e.target.value })} value={MachineInfo.MachineDetails} />
                                ...
                                <div className='col-lg-4 mb-2'>
                                    <label>Status</label>
                                    <select className='form-select  custom-border' type='' onChange={(e) => setMachineInfo({ ...MachineInfo, Status: e.target.value })}
                                        value={MachineInfo.Status} >
                                        <option></option>
                        ...
                        <div className="modal-content card">
                            ...
                            <div className="modal-body">
                                ...
                                <div className="download-sample-template text-center">
                                    ...
                                    <div>
                                        <a href="/Machine_Upload_Temp.xlsx" className=' nav-link text-decoration-underline ' download>Click to Download</a>
                                    </div>
                                </div>
            ...
            <div className='card px-5 pt-2 pb-3'>
                <div className='d-flex justify-content-end flex-wrap '>
                    ...
                    <button className="btn  border-0 btn-success text-white mx-2 mb-2 " onClick={() => setIsModalVisible(true)}>
                        <i className="bi bi-plus-circle me-1"></i> <span className='pt-1'>Add Machine</span>
                    </button>
