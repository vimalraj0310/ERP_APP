...
const Department = ({ auth }) => {
  ...
  const handleCreateRole = async () => {
    ...
    if (isValidationSuccess) {
      swal({
        text: "This Department Name is Already Exitsing",
        icon: "warning"
      });
      return;
    }
    ...
    try {
      ...
      const alldata = { id: '', departmentname: name, createdby: auth.empid, updateby: '', branchid: auth.branchid, mode: 'I' }
  ...
  const handleedit = async (id) => {
    try {
      ...
      const alldata = { id: id, departmentname: '', createdby: auth.empid, updateby: '', branchid: auth.branchid, mode: 'E' }
      const response = await axios.post(`${API_URL}/DepartmentMaster`, alldata)

      if (response.status === 200) {

        SetUpdateName(response.data[0].departmentname)
  ...
  const handleupdate = async () => {
    try {
      ...
      const alldata = { id: editid, departmentname: UpdateName, createdby: '', updateby: auth.empid, branchid: auth.branchid, mode: 'U' }
      ...
      if (response.status === 200) {
        ...
        swal({
          text: 'DepartMent Name Update SuccessFully',
          icon: 'success'
        })
      }
  ...
  const confirmDelete = async () => {
    try {
      ...
      const alldata = { id: editid, departmentname: '', createdby: '', updateby: '', branchid: '', mode: 'D' }
  ...
  const handlesearch = async (e) => {
    try {
      ...
      const filterdata = TableDatas.filter((data) =>
        data.departmentname.toLowerCase().includes(input)
      );
  ...
  return (
    <div>
      ...
      >
        <CModalTitle id="VerticallyCenteredExample" className='ms-3'>{Status === 'Add' ? 'Create New DepartMent' : 'Edit DepartMent Name'}</CModalTitle>
        <CModalBody>
          <CForm>
            <CFormLabel htmlFor="hsn">DepartMent <span style={{ color: 'red' }}>*</span></CFormLabel>
            <CFormInput
              className='mb-3'
              type="text"
              id='name'
              ...
              <CTableBody>
                ...
                {(filteredData.length > 0 ? filteredData : TableDatas).map((d, index) => (
                  <CTableRow key={index}>
                    ...
                    <CTableDataCell className='text-capitalize'>{d.departmentname}</CTableDataCell>
                    <CTableDataCell className='col-sm-2 text-wrao'>
                      <CTooltip content="Edit">
                        <CIcon icon={cilPencil} size='xl' className='mx-2' onClick={() => handleedit(d.departmentid)}
                          style={{ color: 'white', background: 'green', borderRadius: '5px' }}
                        />
                      </CTooltip>
                      {/* <CTooltip content="Delete">
                        <CIcon icon={cilTrash} size='xl' className='mx-2' onClick={() => handledelete(d.departmentid)}
